...
class BaseClothingScreen
  ...
  def select_base_type(type)
    return if selected_type_name('base') == type
  ...
  def select_conversion_type(type_name)
  ...
  def select_clothing_type(type_name)
    return if selected_type_name('category') == type_name

    open_type_category 'category'
    find_category type_name
    type_popup_elements.each do |element|
      if element.text == type_name
        element.click
